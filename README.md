...
Анкеты работодателя по умолчанию остаются на ручной проверке. Опция `AUTO_APPLY_QUESTIONNAIRES=true` разрешает автоматические ответы для явно заданных данных: зарплатных ожиданий, удалённого формата и ссылки на портфолио. Профессиональные текстовые вопросы могут получить короткий ответ через настроенную LLM только по фактам профиля; неполный или небезопасный результат переводит всю анкету на ручную проверку. Юридические, персональные и неизвестные вопросы приходят в Telegram и не останавливают пачку. Если анкета раскрывает компанию, позицию или отрасль из исключений профиля, вакансия отклоняется до финальной отправки.
